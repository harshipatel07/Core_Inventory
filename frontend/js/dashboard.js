...
async function renderDashboard() {
    ...
    content.innerHTML = `<div class="animate-in">
        ...
        <div class="dash-ops-grid" style="margin-bottom:1.5rem">
            ...
            <div class="card" style="cursor:pointer" onclick="navigate('transfers')">
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
                    <div class="kpi-icon purple"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg></div>
                    <h3 style="margin:0;font-size:1rem;font-weight:600">Internal Transfers</h3>
                </div>
                <div id="transfer-card-body" style="display:flex;gap:16px;flex-wrap:wrap"></div>
            </div>
        </div>
    ...
    try {
        ...
        document.getElementById('kpi-grid').innerHTML = `
            ...
            <div class="kpi-card kpi-red" style="cursor:pointer" onclick="navigate('stock')">
                ...
                <div class="kpi-value">${(kpis.low_stock_count || 0) + (kpis.out_of_stock_count || 0)}</div>
                <div class="kpi-label">Low / Out of Stock</div>
            </div>
`;

        // ── Transfers card ──
        document.getElementById('transfer-card-body').innerHTML = `
            <div style="text-align:center;padding:8px 14px;background:var(--bg-secondary);border-radius:8px;min-width:72px">
                <div style="font-size:1.8rem;font-weight:800;color:var(--accent-primary)">${kpis.pending_transfers}</div>
                <div style="font-size:11px;color:var(--text-muted)">Pending</div>
            </div>
            <div style="text-align:center;padding:8px 14px;background:var(--bg-secondary);border-radius:8px;min-width:72px">
                <div style="font-size:1.8rem;font-weight:800;color:#00b894">${kpis.done_transfers || 0}</div>
                <div style="font-size:11px;color:var(--text-muted)">Completed</div>
            </div>
            <div style="text-align:center;padding:8px 14px;background:var(--bg-secondary);border-radius:8px;min-width:72px">
                <div style="font-size:1.8rem;font-weight:800;color:var(--text-secondary)">${kpis.all_transfers || 0}</div>
        ...
        const bt = document.getElementById('badge-transfers');
        if (bt) bt.textContent = kpis.pending_transfers > 0 ? kpis.pending_transfers : '';
